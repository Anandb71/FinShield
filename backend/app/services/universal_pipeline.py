...
class DocumentPipeline:
    ...
    def _cross_document_consistency(
        ...
        """
        Lightweight cross-document consistency checks using the in-memory
        document store. This is intentionally simple but covers the most
        ...
        try:
            ...
            # Bank statement continuity: closing balance of previous statement
            # for the same account should roughly match the opening balance of
            # the new statement.
            if doc_type == "STATEMENT":
                account_number = extracted_data.get("account_number")
                opening = float(str(extracted_data.get("opening_balance", 0)).replace(",", ""))
                if account_number and opening:
                    for other in store.documents.values():
                        other_class = (other.get("classification") or {}).get("type", "").upper()
                        if other_class != "STATEMENT":
                            continue
                        other_fields = other.get("extracted_fields") or {}
                        if other_fields.get("account_number") != account_number:
                            continue
                        prev_closing = float(
                            str(other_fields.get("closing_balance", 0)).replace(",", "")
                        )
                        if prev_closing:
                            diff = abs(prev_closing - opening)
                            if diff > 1.0:
                                messages.append(
                                    f"Opening balance {opening} does not match previous closing balance "
                                    f"{prev_closing} for account {account_number} (difference {diff:.2f})."
                                )
                            else:
                                messages.append(
                                    f"Balances consistent with previous statement for account {account_number}."
                                )
                            break
            ...
            if not messages:
                status = "unknown"
            elif any("does not match" in m for m in messages):
                status = "warning"

            return {"status": status, "messages": messages}

        except Exception as exc:
            # Never break the main pipeline because of cross-document checks.
            print(f"[Pipeline] Cross-document consistency check failed: {exc}")
    ...
    def _build_knowledge_graph(
        ...
        *,
        ...
        # Derive a coarse risk level for the document node for use in
        # visualisations. This is intentionally simple and complements
        # the richer validation structures returned elsewhere.
        if errors:
            risk_level = "HIGH"
        elif warnings or confidence < 0.8:
            risk_level = "MEDIUM"
        else:
            risk_level = "LOW"
    ...
    async def process_document(self, pdf_bytes: bytes, filename: str) -> Dict[str, Any]:
        ...
        classification = ai_result.get("classification", {}) or {}
        doc_conf = classification.get("confidence", 0.0)
        doc_type = (classification.get("type") or "UNKNOWN").upper()
        language = classification.get("language")
        image_quality = classification.get("image_quality_score")
        ...
        # 3b. Cross-document consistency checks
        consistency = self._cross_document_consistency(doc_type, extracted_data)
